...
namespace NzbDrone.Core.Notifications.Pushcut
{
    public class Pushcut : NotificationBase<PushcutSettings>
    {
        ...
        {
            _proxy.SendNotification(MOVIE_GRABBED_TITLE, grabMessage?.Message, GetPosterLink(grabMessage.Movie), GetImdbLink(grabMessage.Movie), Settings);
        }

        public override void OnDownload(DownloadMessage downloadMessage)
        {
            _proxy.SendNotification(downloadMessage.OldMovieFiles.Any() ? MOVIE_UPGRADED_TITLE : MOVIE_DOWNLOADED_TITLE, downloadMessage.Message, GetPosterLink(downloadMessage.Movie), GetImdbLink(downloadMessage.Movie), Settings);
        }

        public override void OnMovieAdded(Movie movie)
        {
            _proxy.SendNotification(MOVIE_ADDED_TITLE, $"{movie.Title} added to library", GetPosterLink(movie), GetImdbLink(movie), Settings);
        }

        public override void OnMovieFileDelete(MovieFileDeleteMessage deleteMessage)
        {
            _proxy.SendNotification(MOVIE_FILE_DELETED_TITLE, deleteMessage.Message, GetPosterLink(deleteMessage.Movie), GetImdbLink(deleteMessage.Movie), Settings);
        }

        public override void OnMovieDelete(MovieDeleteMessage deleteMessage)
        {
            _proxy.SendNotification(MOVIE_DELETED_TITLE, deleteMessage.Message, GetPosterLink(deleteMessage.Movie), GetImdbLink(deleteMessage.Movie), Settings);
        }
        ...
        private string GetImdbLink(Movie movie)
        {
            return $"https://www.imdb.com/title/{movie.ImdbId}";
        }

        private string GetPosterLink(Movie movie)
        {
